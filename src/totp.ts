...
const sha1HotpTrunc = (hash: Buffer) => {
...
const b32ToBuffer = (b32Str: string) => {
...
export const getTotp = (b32Key: string, tIn?: number) => {
  const t = tIn === undefined ? Math.floor(Date.now() / 1000 / 30) : tIn;
  const hash = createHmac("sha1", b32ToBuffer(b32Key))
    .update(getCounter(t))
    .digest();
